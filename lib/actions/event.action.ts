...
import Category from '@/lib/database/models/category.actions'
Category
...
export async function createEvent({ userId, event, path }: CreateEventParams) {
  try {
    ...
    const newEvent = await Event.create({ ...event,capacity:event.capacity, category: event.categoryId, organizer: userId })
    revalidatePath(path)
